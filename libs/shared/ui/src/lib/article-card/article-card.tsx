...
export async function ArticleCard({post}: ArticleCardProps) {
  const image = (await import(/* webpackMode: "eager" */ `cms/assets/${post.core.image.src}`)).default;
  return (
    <Link
      href={post.url}
      className={cn(
      ...
      <div className="space-y-6 w-full">
        ...
        <p
          className="hidden line-clamp-3 sm:line-clamp-4 leading-relaxed text-sm md:text-base"
        >
